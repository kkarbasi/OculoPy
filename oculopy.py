...
class SaccadeDetector:
    """
    Class for detecting saccades from eye position data and target jumps from target position data.
    """
    ...
    def __init__(self,
        ...
        self.VT = np.squeeze(VT)
        self.t_VT = np.squeeze(t_VT)
        self.HE = np.squeeze(HE)
        self.t_HE = np.squeeze(t_HE)
        self.VE = np.squeeze(VE)
        self.t_VE = np.squeeze(t_VE)
        self.fs = eye_fs
        self.dt = 1.0/eye_fs

    def run(self):
        self._cut_to_min_size()
        self._detect_saccades()
        self._detect_target_jumps()
